...
import { createChatExport, downloadChatExport } from '../lib/chat-export';
export const RightPanel = () => {
  ...
  const handleExport = useCallback(() => {
    const payload = createChatExport(chatMessages, projectName);
    downloadChatExport(payload);
  ...
  const activeChatSession = orderedChatSessions.find(session => session.id === activeChatSessionId) ?? null;
  const activeChatSessionState = activeChatSessionId ? chatSessionStates[activeChatSessionId] : null;
  ...
  return (
    <aside className="w-[calc(40%+14rem)] min-w-[624px] max-w-[824px] flex flex-col bg-deep border-l border-border-subtle animate-slide-in relative z-30 flex-shrink-0">
      {/* Header with Tabs */}
      ...
      {activeTab === 'chat' && (
        <div className="flex-1 flex overflow-hidden">
          <div className="w-56 flex-shrink-0 border-r border-border-subtle bg-surface/30 flex flex-col">
            <div className="p-3 border-b border-border-subtle">
              <button
                onClick={createChatSession}
                className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-accent text-white text-sm font-medium transition-colors hover:bg-accent-dim"
              >
            ...
            <div className="flex-1 overflow-y-auto p-2 scrollbar-thin">
              <div className="space-y-2">
                {orderedChatSessions.map((session) => {
                  ...
                  const sessionState = chatSessionStates[session.id];
                  ...
                  return (
                    <div
                      ...
                      className={`group relative rounded-xl border transition-colors ${isActive
                        ...
                        : 'border-transparent bg-elevated/40 hover:border-border-subtle hover:bg-hover'
                      }`}
                    >
                      <button
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <MessageSquare className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isActive ? 'text-accent' : 'text-text-muted'}`} />
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px]">
                              ...
                              {sessionState?.isLoading && (
                                <span className="inline-flex items-center gap-1 rounded-full border border-accent/30 bg-accent/10 px-1.5 py-0.5 text-accent">
                                  <Loader2 className="w-3 h-3 animate-spin" />
                                  <span>Running</span>
                                </span>
                              )}
                              {!sessionState?.isLoading && sessionState?.error && (
                                <span className="inline-flex items-center gap-1 rounded-full border border-rose-500/30 bg-rose-500/10 px-1.5 py-0.5 text-rose-200">
                      ...
                      <button
                        ...
                        className={`absolute top-2 right-2 p-1 rounded text-text-muted transition-colors ${isActive
                          ? 'hover:text-rose-200 hover:bg-rose-500/20'
                          : 'opacity-0 group-hover:opacity-100 hover:text-rose-200 hover:bg-rose-500/20'
                        }`}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex items-center gap-2.5 px-4 py-3 bg-elevated/50 border-b border-border-subtle">
              ...
              <div className="ml-auto flex items-center gap-2">
                {activeChatSessionState?.isLoading && (
                  <span className="text-[11px] px-2 py-1 rounded-full bg-accent/10 text-accent border border-accent/30 flex items-center gap-1">
                    <Loader2 className="w-3 h-3 animate-spin" /> Running
                  </span>
                )}
                {!activeChatSessionState?.isLoading && activeChatSessionState?.error && (
                  <span className="text-[11px] px-2 py-1 rounded-full bg-rose-500/10 text-rose-200 border border-rose-500/30 flex items-center gap-1">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border-subtle bg-surface text-xs font-medium text-text-secondary transition-colors hover:bg-hover hover:text-text-primary disabled:cursor-not-allowed disabled:opacity-50"
                                ...
                                {message.steps.map((step, index) => (
                                  <div key={step.id}>
                                    {step.type === 'reasoning' && step.content && (
                                      <div className="mb-3 rounded-xl border border-border-subtle bg-surface/50 px-3 py-2">
